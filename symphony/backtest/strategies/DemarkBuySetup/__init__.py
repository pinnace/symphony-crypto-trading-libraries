...
class DemarkBuySetup(Strategy):
    ...
    def __init__(self):
        ...
        self.results_helper = ResultsHelper("DemarkBuySetup-Pressure-Demarker", use_s3=False)
        ...
        self.flags = {
            ...
            "TPFib": False,
            "PadStopLoss": True,
            "ExitOnSellSetup": True,
            "PressureAndDemarker": True
        }
    ...
    def before(self):
        ...
        if self.flags["PressureAndDemarker"]:
            self.price_history = td_pressure(self.price_history)
            self.price_history = td_demarker_I(self.price_history)
    ...
    def should_long(self) -> bool:
        if self.__buy_setup_present():
            if self.flags["PressureAndDemarker"] and is_oversold(self.price_history, IndicatorRegistry.TD_DEMARKER_I) and is_oversold(self.price_history, IndicatorRegistry.TD_PRESSURE):
                return True
            return True
        return False
    ...
    def go_long(self):
        ...
        if not self.price_history.price_history.iloc[-1][IndicatorRegistry.BUY_SETUP.value]:
